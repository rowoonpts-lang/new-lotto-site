<?
	...
	$sql_common = " and result not in ('','낙첨') and sms_yn is null and (mb_type not in ('무료회원') or (mb_type in ('무료회원') and result in ('1등','2등','3등','4등') )) ";
	...
	if($list['returnValue'] != 'fail'){
		...
		foreach($ary as $key => $value) {
			...
			if($tot > 0){
				...
				// SMS 발송
				fnSendOneshot($config['cf_oneshot_tel'], $value['mb_hp'], $msg , $config['cf_oneshot_080']);
