<?
	...
	$sql = "select * from l_turn_{$recent_turn} where 1=1 and result = '4등' limit 1";
	$row = sql_fetch($sql);
	...
	echo "<br>".$sql;
	sql_query($sql);
	$sql = "update g5_member_etc set set4grade = '{$recent_turn}' where mb_id in ({$result_mb_id}) ";
	echo "<br>".$sql;
	sql_query($sql);
