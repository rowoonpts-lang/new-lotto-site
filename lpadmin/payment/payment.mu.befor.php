		...
		<div class="card">
			...
			<div class="card-body table-responsive p-0">
				...
				<tr>
					...
					<td><?=$row['mu_mb_name']?></td>
					<td>
						<?php if($row['lp_status'] == "주문"){?>
						대기
						<?php }?>
					</td>
					...
					<td>
						<?php if($row[lp_status] == "주문"){?>
						<button type="button" class="btn btn-success" onClick="fnPayMu('<?=$row[lp_id]?>')">입금확인</button>
						<?php }?>
					</td>
					<td>
...
var cancleMu = true;
function fnExcel(){
	location.href="./payment.mu.before.excel.php?1=1<?=$qstr?>";
}
...
function fnPayMu(lp_id){
	...
	if(confirm("해당 결제건을 입급확인 하시겠습니까?") == true){
		cancleMu = false;
		location.href="<?=G5_LADMIN_URL?>/member/payment.mu.pay.php?lp_id="+lp_id
		/*$.ajax({
			type: "POST",
