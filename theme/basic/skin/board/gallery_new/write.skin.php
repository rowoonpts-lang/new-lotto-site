...
<section id="bo_w">
    ...
    <div class="write_div">
        ...
        <?php echo $option ?>
		<?php if($bo_table == "talk_talk"){?>
			<input type="checkbox" id="wr_2" name="wr_2" value="Y" <?php if($wr_2 == "Y" ){?>checked<?php }?>>
			<label for="wr_2">메인노출여부</label>   
		<?php }?>
    </div>
    <?php } ?>
	...
	<div class="write_div">
		<label for="wr_name" class="sound_only">이름<strong>필수</strong></label>
        <input type="text" name="wr_10" value="<?php echo $write[wr_10] ?>" id="wr_10" class="frm_input full_input" placeholder="해시태그 (ex. #교육 #창직교육)">
	</div>
    ...
    <?php } ?>
	<p>배너추가</p>
	<div class="bo_w_link write_div bo_w_select ">
        <label for="wr_9"><i class="fa fa-link" aria-hidden="true"></i><span class="sound_only"> 링크  #<?php echo $i ?></span></label>
        <input type="text" name="wr_9" value="<?php if($w=="u"){echo$write['wr_9'];} ?>" id="wr_9" class="frm_input full_input" size="50">
    </div>
	...
	<div class="bo_w_flie write_div">
        <div class="file_wr write_div">
            <label for="bf_file_2" class="lb_icon"><i class="fa fa-download" aria-hidden="true"></i><span class="sound_only"> 파일 #<?php echo $i+1 ?></span></label>
            <input type="file" name="bf_file[2]" id="bf_file_2" title="파일첨부 2 : 용량 <?php echo $upload_max_filesize ?> 이하만 업로드 가능" class="frm_file ">
        </div>
        <?php if ($is_file_content) { ?>
        <input type="text" name="bf_content[2]" value="<?php echo ($w == 'u') ? $file[2]['bf_content'] : ''; ?>" title="파일 설명을 입력해주세요." class="full_input frm_input" size="50" placeholder="파일 설명을 입력해주세요.">
        <?php } ?>
