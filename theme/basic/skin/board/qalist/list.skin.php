...
{
	$row = get_list($row, $board, $g4[path].$board_skin_url, $board[bo_subject_len]);
...
$bb_total_count = sql_num_rows($bb_result_total);
...
{
	$row = get_list($row, $board, $board_skin_url, $board[bo_subject_len]);
	array_push($list2A, $row);
}
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            ...
            if ($list[$i]['is_notice']){ // 공지사항
				...
				if(!$list[$i][wr_reply]){
					echo '<img src="'.$board_skin_url.'/img/icon_comment.png">';
				}
                //echo $list[$i]['num'];
			}
             ?>
